...
from stm32_agent.service import (
    ChatReply,
    ChatSession,
    GenerationRun,
...
def _app_stylesheet() -> str:
    ...
    #ChatBubbleAI {{
        ...
        color: {TEXT};
        border: 1px solid rgba(190, 204, 220, 0.6);
        border-radius: 18px;
        border-bottom-left-radius: 6px;
        padding: 12px 16px;
    }}
    #ChatFileCard {{
        background: rgba(48, 185, 95, 0.08);
        border: 1px solid rgba(48, 185, 95, 0.35);
        border-radius: 12px;
        padding: 8px 14px;
    }}
    #ChatInput {{
...
class ChatBubble(QWidget):
    ...
    def __init__(self, text: str, *, is_user: bool, parent: QWidget | None = None) -> None:
        ...
        role_label = QLabel("你" if is_user else "AI 助手")
        role_label.setObjectName("ChatRoleLabel")

        col = QVBoxLayout()
        col.setContentsMargins(0, 0, 0, 0)
        col.setSpacing(4)
        col.addWidget(role_label)
        ...
        if is_user:
            row.addStretch(1)
            col.setAlignment(role_label, Qt.AlignRight)
        row.addLayout(col)
        if not is_user:
            row.addStretch(1)


class FileUpdateCard(QWidget):
    """文件修改提示卡片。"""

    def __init__(self, file_path: str, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        card = QFrame()
        card.setObjectName("ChatFileCard")
        inner = QHBoxLayout(card)
        inner.setContentsMargins(10, 6, 10, 6)
        icon_label = QLabel("📝")
        path_label = QLabel(f"<b>{file_path}</b> 已更新")
        path_label.setWordWrap(True)
        inner.addWidget(icon_label)
        inner.addWidget(path_label, 1)
...
class ChatWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # 占位提示
        self.placeholder = QLabel("请先生成项目后再开始 AI 对话")
        self.placeholder.setObjectName("ChatPlaceholder")
        ...
        self.messages_layout.setContentsMargins(6, 10, 6, 10)
    ...
    def activate_session(self, session: ChatSession) -> None:
        """设置 ChatSession 并切换到对话模式。"""
        self._session = session
        self.placeholder.hide()
        self.scroll_area.show()
        self.input_bar.show()
        self._clear_messages()
        self._add_system_bubble("AI 对话已就绪。你可以对已生成的项目提出修改意见、提问或优化请求。")

    def get_api_config(self) -> dict | None:
        """由主窗口在调用前注入。"""
        return getattr(self, "_api_config", None)

    def set_api_config(self, api_key: str, base_url: str, model: str, timeout: int) -> None:
        self._api_config = {
            "api_key": api_key,
            "base_url": base_url,
            "model": model,
    ...
    def _on_send(self) -> None:
        ...
        if self._session is None:
            QMessageBox.information(self, "提示", "请先生成项目后再开始对话。")
            return
        config = self.get_api_config()
        if not config:
            QMessageBox.warning(self, "缺少配置", "请在左侧填写 API Key 和模型信息。")
            return
    ...
    def _on_reply(self, reply: object) -> None:
        ...
        self._add_bubble(reply.text, is_user=False)
        for mf in reply.modified_files:
            self._add_file_card(mf.path)

    def _on_error(self, detail: str) -> None:
        self._set_sending(False)
        self._add_bubble(f"⚠️ 请求失败：{detail}", is_user=False)
    ...
    def _on_clear(self) -> None:
        ...
        self._session = ChatSession(run)
        self._clear_messages()
        self._add_system_bubble("对话已清空，上下文已重置。")
    ...
    def _set_sending(self, sending: bool) -> None:
        ...
        self.input_edit.setDisabled(sending)
        self.send_button.setText("发送中…" if sending else "发送")
    ...
    def _add_file_card(self, file_path: str) -> None:
        card = FileUpdateCard(file_path)
...
class AgentWindow(QMainWindow):
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self.tabs.addTab(self.chat_widget, "AI 对话")
    ...
    def _handle_success(self, run: object) -> None:
        ...
        # 初始化 AI 对话 session
        self.chat_session = ChatSession(run)
        timeout_text = self.timeout_edit.text().strip() or "120"
        try:
            timeout_val = int(timeout_text)
        except ValueError:
            timeout_val = 120
        self.chat_widget.set_api_config(
            api_key=self.api_key_edit.text().strip(),
            base_url=self.base_url_edit.text().strip(),
            model=self.model_edit.text().strip() or DEFAULT_MODEL,
            timeout=timeout_val,
        )
        self.chat_widget.activate_session(self.chat_session)
        self._append_log("AI 对话已就绪，可切换到「AI 对话」标签页进行交互。")

        QMessageBox.information(self, "完成", f"结果已生成到：\n{run.output_dir}\n\n你现在可以切换到「AI 对话」标签页与 AI 交互，继续打磨项目。")
