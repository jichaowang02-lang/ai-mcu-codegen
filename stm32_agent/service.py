...
_CHAT_SYSTEM_PROMPT = """\
你是一个嵌入在 STM32 Agent Studio 中的 AI 助手。用户已经使用本工具生成了一组面向 STM32CubeMX + HAL + Keil5 的工程文件。
现在，用户可以通过多轮对话继续与你交互，以打磨和优化这些文件。
...
当你需要修改文件时，请在回复末尾使用下面的格式（可以包含多个文件块）：

<<<FILE_UPDATE>>>
PATH: 相对路径（如 Core/Src/main.c）
CONTENT:
完整的文件新内容
<<<END_FILE_UPDATE>>>

规则：
- 每次修改请给出完整的文件新内容，不要使用省略。
- 如果不需要修改文件，直接回复文字即可，不要输出 FILE_UPDATE 块。
- 回答时使用中文。
"""
...
class ChatSession:
    ...
    def __init__(self, run: GenerationRun) -> None:
        self.run = run
    ...
    def _init_context(self) -> None:
        context_parts = [
            ...
            "",
            "以下是当前项目的概况：",
            f"- 项目名：{self.run.result.project_name}",
            f"- MCU：{self.run.result.target_mcu}",
            f"- 生成模式：{self.run.result.generation_mode}",
            f"- 输出目录：{self.run.output_dir}",
            "",
            "已生成的文件清单：",
        ]
        for f in self.run.result.files:
            context_parts.append(f"  - {f.path}（{f.purpose}）")

        # 把每个文件的内容也带上，让 AI 能看到完整项目
        context_parts.append("")
        context_parts.append("以下是每个文件的完整内容：")
        for f in self.run.result.files:
...
_FILE_UPDATE_PATTERN = re.compile(
    r"<<<FILE_UPDATE>>>\s*\nPATH:\s*(.+?)\s*\nCONTENT:\s*\n(.*?)<<<END_FILE_UPDATE>>>",
    re.DOTALL,
)
...
def send_chat_message(
    ...
    *,
    ...
    reply_text = client.complete_text(session.messages)
    session.messages.append({"role": "assistant", "content": reply_text})

    # 解析文件修改
    modified: list[ModifiedFile] = []
    for match in _FILE_UPDATE_PATTERN.finditer(reply_text):
        file_path = match.group(1).strip()
        file_content = match.group(2).strip()
        modified.append(ModifiedFile(path=file_path, content=file_content))

    # 写回磁盘
    for mf in modified:
        target = session.run.output_dir / mf.path
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(mf.content, encoding="utf-8")

    # 剥离 FILE_UPDATE 标记后的纯文本
    display_text = _FILE_UPDATE_PATTERN.sub("", reply_text).strip()
    if not display_text and modified:
        display_text = f"已更新 {len(modified)} 个文件。"

    return ChatReply(text=display_text, modified_files=modified)
